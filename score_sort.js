...
if (orderType == '0' || orderType == '1') {
  ...
  for (let i = 1; i <= nodes.length; i++) {
    tmpScore = nodes[i - 1].querySelectorAll(
      '.playdata__score-list__song-info__score'
    )

    $(nodes[i - 1]).data(
      'rank_normal_score',
      tmpScore[0].innerText.replace(/\s+/g, '').replace('SCORE', '')
    )
    $(nodes[i - 1]).data(
      'rank_hard_score',
      tmpScore[1].innerText.replace(/\s+/g, '').replace('SCORE', '')
    )
    $(nodes[i - 1]).data(
      'rank_expert_score',
      tmpScore[2].innerText.replace(/\s+/g, '').replace('SCORE', '')
    )
    $(nodes[i - 1]).data(
      'rank_inferno_score',
      tmpScore[3].innerText.replace(/\s+/g, '').replace('SCORE', '')
    )
  }
